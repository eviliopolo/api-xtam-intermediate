...
const getAndUpdateRecording = async (
  id_xtam: string,
  date: Date,
...
const getRecordingsOne = async (id_xtam: string, date: Date) => {
  const response = await recordingsModel.findOne({
    id_xtam: id_xtam,
    date: date,
  });
...
const updateRecordings = async (id: string, recording: recordings) => {
  const response = await recordingsModel.findByIdAndUpdate(
    { _id: id },
    recording,
    { new: true }
  );
...
const deleteRecording = async (id: string) => {
  const response = await recordingsModel.findByIdAndRemove(
    { _id: id },
    { new: true }
  );
  return response;
...
const estructureInfoApi = async (objetc: any, data: any,id_xtam:any) => {
  ...
  let searchInfo = objetc.filter(
    (n: any) =>
      n.data_time_start >= data.dateStart &&
      n.data_time_finish <= data.dateFinish
  );
  ...
  const resApi = await axios.post(
    `${process.env.URL_API_RECORDINGS}/api/stremingRecording/`,
    dataSend
  );
...
const getRecordingsFilter = async (
  id_xtam: string,
  date: Date,
  data: any,
  dateRedis: string
  ...
  console.log(dateToday, dateTodayEstructure);
  const daysDiffe = dateTodayEstructure.diff(dateTodayFinish, "minutes");
  console.log("diff", daysDiffe);
  ...
  if (daysDiffe < 30 && daysDiffe > 0) {
    ...
    const response: any = await recordingsModel.findOne({
      id_xtam: id_xtam,
      date: date,
    });
...
const timeAvailed = async (
  id_xtam: string,
  dateStart: any,
  dateFinish: any,
  folder_record: string
) => {
  ...
  if (daysDiff < 0) {
    ...
    const response: any = await recordingsModel.find({
      id_xtam: id_xtam,
      $and: [{ date: { $gte: dateInit } }, { date: { $lte: dateEnd } }],
    });
...
const timeAvailedForDay = async (
  ...
  allDates.map(async (element: any, index: any) => {
    let searchInfo = tmpHisto.filter(
      (n: any) =>
        n.data_time_start.slice(11, 17) >= allDates[index] &&
        n.data_time_finish.slice(11, 17) <= allDates[index + 1]
    );
